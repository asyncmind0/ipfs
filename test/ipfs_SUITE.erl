...
add_file(_Config) ->
  ...
  {ok, #{<<"Hash">> := ?DATA_1MiB_HASH, <<"Size">> := <<"1048832">>}} = ipfs:add(Pid, TmpFile),
  file:delete(TmpFile),
  ipfs:stop(Pid).
...
sh_data(Port, Timeout, Acc) ->
  receive
    {Port, {exit_status, Status}} -> {Status, erlang:iolist_to_binary(lists:reverse(Acc))};
    {Port, {data, {eol, Line}}} -> sh_data(Port, Timeout, ["\n", Line | Acc])
  after
    Timeout ->
      exit(Port, kill),
...
sh_result(Status, Output, Allowed) ->
  case lists:member(Status, Allowed) of
    true -> {Status, Output};
    false -> erlang:error({sh, Status, Output})
  end.


mktemp() -> filename:join([<<"/tmp">>, integer_to_binary(erlang:system_time())]).
